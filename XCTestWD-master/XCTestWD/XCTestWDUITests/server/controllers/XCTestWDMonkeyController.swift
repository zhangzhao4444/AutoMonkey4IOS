...
internal class XCTestWDMonkeyController: Controller {
    ...
    internal static func swiftmonkey(request: Swifter.HttpRequest) -> Swifter.HttpResponse {
        ...
        let username=desiredCapabilities?["username"]?.string ?? nil
        let password=desiredCapabilities?["password"]?.string ?? nil
        if bundleID == nil {
        ...
        if bundleID == "com.egbank.EGBankP2B-Dev" {
            monkey.addXCTestPasswordAction(interval:30, application:app)
        }
        monkey.addXCTestAppMain(interval: 150, application: app)
        monkey.addXCTestCurrentPage(interval: 25, application: app)
        if username != nil && password != nil {
            monkey.addXCTestAppLogin(interval: 60, application: app,username:username!,password:password!)
        }
        monkey.monkeyAround()
